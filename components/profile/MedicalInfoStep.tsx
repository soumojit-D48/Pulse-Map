  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Help us understand your donation eligibility</p>
      </div>
      ...
      <form id="medical-info-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {bloodGroups.map((group) => (
              <label
                key={group.value}
                className="flex flex-col items-center justify-center px-4 py-4 border-2 border-border rounded-lg cursor-pointer hover:border-primary has-[:checked]:border-primary has-[:checked]:bg-primary/10 transition-all"
              >
                <input
                  type="radio"
                  value={group.value}
                ...
                <Droplet 
                  className={`w-6 h-6 mb-1 ${
                    selectedBloodGroup === group.value ? 'text-primary' : 'text-muted-foreground'
                  }`}
                />
                <span className="text-lg font-bold text-foreground">{group.label}</span>
              </label>
            ))}
          </div>
          {errors.bloodGroup && (
            <p className="mt-1 text-sm text-destructive">{errors.bloodGroup.message}</p>
          )}
        </div>

        {/* Last Donation Date */}
        <div>
          <label htmlFor="lastDonationDate" className="block text-sm font-medium text-foreground mb-1">
            Last Donation Date <span className="text-muted-foreground">(Optional)</span>
          </label>
          <input
            id="lastDonationDate"
            type="date"
          ...
          {errors.lastDonationDate && (
            <p className="mt-1 text-sm text-destructive">{errors.lastDonationDate.message}</p>
          )}
        ...
        {eligibilityStatus && (
          <div className={`p-4 rounded-lg ${
            ...
            <p className={`text-sm font-medium ${
              eligibilityStatus.eligible ? 'text-success' : 'text-destructive'
            }`}>
        ...
        <div className="p-4 bg-accent/10 border border-accent rounded-lg">
          <h4 className="font-semibold text-accent mb-2">Blood Donation Facts:</h4>
        ...
        <div className="p-4 bg-muted rounded-lg border border-border">
          <h4 className="font-semibold text-foreground mb-3">Blood Compatibility Guide:</h4>
          <div className="text-sm text-card-foreground space-y-2">
            <div className="flex justify-between">
              <span className="font-medium">O-</span>
              <span className="text-muted-foreground">Universal donor (can donate to all)</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">AB+</span>
              <span className="text-muted-foreground">Universal receiver (can receive from all)</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">A+</span>
              <span className="text-muted-foreground">Can donate to A+, AB+</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">B+</span>
              <span className="text-muted-foreground">Can donate to B+, AB+</span>
            </div>
          </div>
        </div>
      </form>
